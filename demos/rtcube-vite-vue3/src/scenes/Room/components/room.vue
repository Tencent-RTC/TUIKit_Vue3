<template>
  <ConferenceMainView class="room-main" />
</template>
...
// Hide ThemeRegistrar, LanguageRegistrar, LoginUserInfoRegistrar
conference.setWidgetVisible({
  [BuiltinWidget.ThemeWidget]: false,
  [BuiltinWidget.LanguageWidget]: false,
  [BuiltinWidget.LoginUserInfoWidget]: false,
});
...
async function handleStartConference() {
  const roomName = `${loginUserInfo.value?.userId || 'User'}${t('Room.TemporaryMeeting') || "'s meeting"}`;

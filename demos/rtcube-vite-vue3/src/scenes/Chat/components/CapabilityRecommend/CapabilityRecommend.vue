...
import { IconLogo, TUIButton, useUIKit } from '@tencentcloud/uikit-base-component-vue3';
...
import { reportSceneSelect, reportLinkClick } from '@/utils/aegis';
...
const handleExperienceDemo = (cardId: string, scene: string) => {
  // Report capability card demo click (remove for GitHub demo)
  // ext1 format: "detail | targetScene" (e.g., "detail | call")
  reportLinkClick('detail', 'chat', 'experience', cardId);
  emit('switchScene', scene);
};

const handleViewDocs = (cardId: string, url: string) => {
  // Report capability card docs click (remove for GitHub demo)
  // ext1 format: "detail | chat | doc | url"
  reportLinkClick('detail', 'chat', 'doc', url);

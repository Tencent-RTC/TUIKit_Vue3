...
import { reportLinkClick } from '@/utils/aegis';
import { AEGIS_PAGES } from '@/constants/aegis';

const { t } = useUIKit();

/**
 * Report quick access link click event (remove for GitHub demo)
 * ext1 format: "home | product | platform | url" (e.g., "home | chat | web | https://...")
 */
const handleLinkClick = (productId: string, platform: string, url: string) => {
  reportLinkClick(AEGIS_PAGES.HOME, productId, platform, url);
};

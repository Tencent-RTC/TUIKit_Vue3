...
/**
 * Sub-scene configuration interface
 */
export interface SubSceneConfig {
  /** Sub-scene identifier */
  id: string;
  /** i18n key for label */
  labelKey: string;
  /** i18n key for description */
  descKey: string;
  /** Whether this is the default sub-scene */
  isDefault?: boolean;
}
...
export interface SceneConfig {
  ...
  imageUrl?: string;
  /** Optional sub-scenes for this scene */
  subScenes?: SubSceneConfig[];
}
    ...
    subScenes: [
      {
        ...
        descKey: 'scenes.chat.subScenes.general.desc',
        isDefault: true,
      },
      {
        id: 'medical',
        labelKey: 'scenes.chat.subScenes.medical.label',
        descKey: 'scenes.chat.subScenes.medical.desc',
      },
    ],
  },
  {
...
/**
 * Get sub-scenes for a given scene
 */
export function getSubScenes(sceneId: SceneId): SubSceneConfig[] {
  const config = getSceneConfig(sceneId);
  return config?.subScenes ?? [];
}

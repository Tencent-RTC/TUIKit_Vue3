<template>
  <TUIDialog
    ...
    :title="title"
    :customClasses="['online-video-dialog']"
  >
...
const emits = defineEmits<{
  ...
  updateOnlineVideoMaterial: [payload: { url: string; playoutVolume: number; networkCacheSizeKB: number }];
  close: [];
}>();
...
/**
 * Submit-in-flight guard. See LocalVideoDialog.vue for the full rationale.
 *
 * Set synchronously before emitting add/update and reset via the exposed
 * `resetSubmitting()` when the parent catches a failure. Prevents duplicate
 * adds when the underlying SDK call is slow (network probe / decoder init).
 */
...
function handleConfirm() {
  ...
  if (isEditMode.value) {
    emits('updateOnlineVideoMaterial', {
      url,
 ...
 * See LocalVideoDialog.vue for the symmetric implementation.
 */

...
const {
  publishVideoQuality, activeMediaSource,
  enableLocalVideoMixer, mediaSourceList,
  clearMediaSource,
} = useVideoMixerState();
...
function getMixControlStyle() {
  ...
  // Use Electron format: rect is a direct field, not nested in layout
  const { rect } = activeMediaSource.value;
  if (!rect) {
    return;
  }
  ...
  const rotation = activeMediaSource.value.rotation || TRTCVideoRotation.TRTCVideoRotation0;
...
onMounted(() => {
  ...
  TUIRoomEngine.once('ready', async () => {
    ...
    if (mediaSourceManager) {
      await mediaSourceManager.bindPreviewArea(0, mixerView as HTMLElement);
    } else {
      console.warn('[LocalMixer] getMediaMixingManager return null or undefined');
    }
    getMixControlStyle();
  });

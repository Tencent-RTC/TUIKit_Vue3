import { TUILiveLayoutManagerEvents, TUIDeviceStatus } from '@tencentcloud/tuiroom-engine-electron';
import { TRTCStreamLayoutMode, TRTCVideoFillMode, TRTCVideoResolution, TRTCVideoResolutionMode } from 'trtc-electron-sdk';
import { useRoomEngine } from '../../hooks/useRoomEngine';
import { deepClone } from '../../utils';
import logger from '../../utils/logger';
import { TUISeatLayoutTemplate } from './type';
import type { TUIVideoResolutionType, TUIUserOnSeatInfo, TUICoHostLayoutTemplate } from './type';
import type { TUISeatLayout, TUISeatRegion } from '@tencentcloud/tuiroom-engine-electron';
import type { TRTCStreamLayout } from 'trtc-electron-sdk';

type StreamLayoutEvent = 'onUserOnSeatInfoChanged' | 'onStreamLayoutChanged' | 'onStreamLayoutAreaChanged';

const roomEngine = useRoomEngine();

const initResolutionMap = (): TUIVideoResolutionType => {
  const map: TUIVideoResolutionType = {};
  Object.keys(TRTCVideoResolution).forEach((key: string) => {
    const isValueProperty = parseInt(key, 10) >= 0;
    if (!isValueProperty) {
      const value = TRTCVideoResolution[key as keyof typeof TRTCVideoResolution];
      const tmp = key.split('_');
      map[value] = {
        width: parseInt(tmp[1], 10),
        height: parseInt(tmp[2], 10),
      };
    }
  });
  return map;
...
const resolutionMap: TUIVideoResolutionType = initResolutionMap();
...
export class StreamLayoutService {
  ...
  private userOnSeatInfos: Array<TUIUserOnSeatInfo>;

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private eventListeners: Map<StreamLayoutEvent, Array<(...args: any[]) => void>>;
  ...
  setRoomInfo(options: { roomId: string; roomOwner: string }): void {
    const changed = this.roomId !== options.roomId || this.roomOwner !== options.roomOwner;
    if (changed) {
  ...
  private onSeatLayoutChanged(options: { roomId: string; seatLayout: TUISeatLayout }) {
    ...
    if (roomId !== this.roomId) {
      logger.warn(`${this.logPrefix}onSeatLayoutChanged received roomId:${roomId}, current roomId:${this.roomId}`);
      return;
    }
  ...
  private setStreamLayout(): void {
    ...
    if (this.layoutTemplateId !== null && this.roomId) {
    ...
    if (mediaMixingManager) {
      ...
      this.emit('onStreamLayoutChanged', deepClone(this.streamLayout));
      this.emit('onUserOnSeatInfoChanged', deepClone(this.userOnSeatInfos));
    } else {
      logger.warn(`${this.logPrefix}setStreamLayout failed, no mediaMixingManager.`);
    }
  }
  ...
  private updateLayoutArea() {
    if (this.container) {
      ...
      this.emit('onStreamLayoutAreaChanged', deepClone(this.layoutArea));
    } else {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  on(event: StreamLayoutEvent, callback: (...args: any[]) => void): void {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  off(event: StreamLayoutEvent, callback: (...args: any[]) => void): void {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private emit(event: StreamLayoutEvent, ...args: any[]): void {
    const listeners = this.eventListeners.get(event);
    if (listeners) {
      listeners.forEach((callback) => {
        try {
          ...
          logger.error(`${this.logPrefix}emit event "${event}" error:`, error);
        }
      });
    }
  }

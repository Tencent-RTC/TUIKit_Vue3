...
function createDragHandlers(el: HTMLElement, binding: DirectiveBinding<FloatDragOptions>): DragState {
  ...
  state.handleTouchStart = (e: TouchEvent) => {
    ...
    // Prevent default and stop propagation to avoid conflict with parent scrollable components
    e.preventDefault();
    e.stopPropagation();
  };

  state.handleTouchMove = (e: TouchEvent) => {
    if (!state.isDragging || state.options.disabled) {
      return;
    }
  ...
  state.handleTouchEnd = (e: TouchEvent) => {
    ...
    // Stop propagation on touch end as well
    e.stopPropagation();
  };
  ...
  state.handleMouseUp = (e: MouseEvent) => {
    ...
    // Stop propagation on mouse up as well
    e.stopPropagation();
  };

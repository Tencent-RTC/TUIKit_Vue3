...
import { useUIKit, TUIButton, TUISlider, TUIToast } from '@tencentcloud/uikit-base-component-vue3';
import { useDeviceState } from '../../states/DeviceState';
import { getSpeakerTestUrl } from '../../utils/speakerTestUrl';
...
async function handleSpeakerTest() {
  const isStartAction = !isSpeakerTesting.value;
  try {
    ...
    console.error('[AudioSettingPanel] speaker test action failed:', error);
    if (isStartAction) {
      TUIToast.error({ message: t('AudioSettingPanel.SpeakerTestStartFailed') });
    }
  }
}
...
const volumeNum = computed(() => {
  if (isMicrophoneTesting.value) {
    return (testingMicVolume.value * volumeTotalNum) / 100;
  }
  return (currentMicVolume.value * volumeTotalNum) / 100;
});

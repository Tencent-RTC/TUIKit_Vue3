...
//
// Implementation notes (Electron kit specific):
//   - `useBattleState()`, `useCoHostState()`, `useLoginState()` here are
//     all pure getters in this kit — they only `return` module-level refs
//     and functions, registering NO `watch` side effects on each call. So
//     calling them at module top level is safe (unlike the Web kit, where
//     the same hooks register `vueWatch` and would leak).
//   - `loginUserInfo` returned by `useLoginState()` is itself a module-level
//     `ref`, so reading `.value` outside any component instance is fine.
//   - Subscription handlers are registered ONCE here. There is no paired
//     `unsubscribe`: the kit's CoHostState lives for the whole page lifetime
//     and HMR teardown is out of scope for this fix.
//
// Mirrors the same pattern in `uikit-component-vue3/.../BattlePanel.vue`
...
const { requestBattle: moduleRequestBattle, subscribeEvent: subscribeBattleEventAtModule } = useBattleState();
const { loginUserInfo: moduleLoginUserInfo } = useLoginState();
const {
  subscribeEvent: subscribeCoHostEvent,
  connected: moduleConnected,
  coHostStatus: moduleCoHostStatus,
} = useCoHostState();
...
const getConnectedAccepterIds = (): string[] => {
  const connectedUserIds = new Set(moduleConnected.value.map(user => user.userId));
...
const fireAggregatedBattle = async (userIdList: string[], duration: number) => {
  ...
  try {
    ...
    // session-boundary watchers below.
  } catch (error) {
...
const onCoHostRequestTimeoutAtModule = ({ inviter, invitee }: { inviter: SeatUserInfo; invitee: SeatUserInfo }) => {
  ...
  // invitee side and have nothing in the pending map.
  if (inviter.userId === moduleLoginUserInfo.value?.userId) {
    pendingBattleRequests.delete(invitee.userId);
...
subscribeBattleEventAtModule(BattleEvent.onBattleStarted, onBattleStartedAtModule);
subscribeBattleEventAtModule(BattleEvent.onBattleEnded, onBattleEndedAtModule);
...
// Module-level `watch` (no auto-dispose) is intentional and matches the rest
// of this block's "lives for the whole page" subscriptions.
...
// `useBattleState` / `useCoHostState` are already imported in the sibling

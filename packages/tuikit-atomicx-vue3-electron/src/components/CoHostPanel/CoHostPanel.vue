    ...
    <div class="panel-content">
      ...
      <div class="panel-body">
        <div v-show="activeTab === 'battleTab'" class="battle-content">
          <BattlePanel :currentLiveOrientation="currentLiveOrientation" :battleDuration="configForm.battleDuration" :coHostLayoutTemplate="configForm.coHostLayoutTemplate" />
        </div>
        <div v-show="activeTab === 'coHostTab'" class="coHost-content">
          <ConnectionPanel :currentLiveOrientation="currentLiveOrientation" :battleDuration="configForm.battleDuration" :coHostLayoutTemplate="configForm.coHostLayoutTemplate" />
        </div>
      </div>
    </div>
    <template #footer>
...
//      (the bug was: form stuck on `HostDynamicGrid` while landscape only
//      offers `HostVideoLandscapeFixed2Seats`, leaving nothing selected).
...
// disconnected). The kit's connection user-list event handler only removes
...
// re-invited, without the user having to close/reopen the panel. Gated on
// `props.visible` so we don't burn rate-limit budget while the panel is
// closed.
//

...
const { userActions, isTargetUserAnchor, isTargetUserAudience, isUserApplyingToAnchor, agreeOrDenyStageList, inviteStageList, onStageControlList } = useUserActions({
  userInfo: props.userInfo,
});

watch(() => JSON.stringify(userActions.value), (newVal) => {
  console.log('lixin-debug aaa userActions', newVal);
}, {
  deep: true,
  immediate: true,
});
...
const singleControl = computed(() => {
  return isMe.value ? null : userActions.value.slice(0, 1)[0];
});

const moreControlList = computed(() => {
  return isMe.value ? userActions.value : userActions.value.slice(1);
});

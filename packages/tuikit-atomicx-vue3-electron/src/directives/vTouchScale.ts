...
class TouchScale {
  ...
  public touchStartPosition = { x: 0, y: 0 };
  public readonly moveThreshold = 10; // pixels
  ...
  private handleTouchStart(event: TouchEvent) {
    ...
    // Only prevent default if already scaled or multi-touch
  ...
  private handleTouchMove(event: TouchEvent) {
    ...
    if (!this.hasMoved && event.touches.length === 1) {
      const currentX = event.touches[0].pageX;
      const currentY = event.touches[0].pageY;
      const deltaX = Math.abs(currentX - this.touchStartPosition.x);
      const deltaY = Math.abs(currentY - this.touchStartPosition.y);
    ...
    // Only prevent default if user has moved, already scaled, or multi-touch
    if (this.hasMoved || this.currentScale !== 1 || event.touches.length > 1) {
      event.preventDefault();
      event.stopPropagation();
    }
  ...
  private handleTouchEnd(event: TouchEvent) {
    ...
    // Only prevent default if user has moved, already scaled, or multi-touch
    if (this.hasMoved || this.currentScale !== 1 || this.isTwoFingerControl) {
      event.preventDefault();
      event.stopPropagation();
    }

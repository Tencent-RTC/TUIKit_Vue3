<template>
  <View>
    <div @click="handleVideoCallClick">
      <slot>
        <div
          :class="cs(styles['video-call-picker__button'], {
            [styles['disabled']]: props.disabled,
          })"
        >
          <IconVideoDefault
            :size="props.iconSize"
            :class="styles['video-call-picker__icon']"
          />
        </div>
      </slot>
    </div>
...
const props = withDefaults(defineProps<VideoCallPickerProps>(), {
  label: '',
  iconSize: 20,
  disabled: false,
});
...
function handleVideoCallClick() {
  ...
  if (isPrivateConversation.value) {
    initiatePrivateCall();
  } else {
    showGroupCallDialog();
  }

...
function getFirstChar(str: string): string {
  ...
  if (/[\u4e00-\u9fa5]/.test(firstChar)) {
    return normalizeLetterGroup(getChinesePinyinFirstLetter(firstChar));
  }
...
function normalizeLetterGroup(char: string): string {
  const matchedLetter = char.toUpperCase().match(/[A-Z]/);

  return matchedLetter?.[0] ?? char.toUpperCase();
}

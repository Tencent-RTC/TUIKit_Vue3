...
export function usePlayerControlState(): PlayerControlState {
  ...
  const initializeResolution = async (roomId: string, applyResolution = true): Promise<void> => {
    try {
      ...
      // Preserve user's resolution choice if it is still available in the new list
      if (currentResolution.value !== undefined && availableResolutions.includes(currentResolution.value)) {
        return;
      }

      // Otherwise default to the first available resolution

...
const pendingInitialScrollTask = ref<PendingInitialScrollTask>(null);
...
async function completeInitialScrollForEmptyList() {
  pendingInitialScrollTask.value = null;
  await nextTick();
...
const initializeMessageList = async () => {
  ...
  if (locateInfo && locateInfo.conversationID === activeConversationID.value) {
    ...
    pendingInitialScrollTask.value = {
      type: 'locate',
      messageID: locateInfo.messageID,
    };
    await loadMessages({
      ...
      .catch(async () => {
        ...
        if (!didInitialScroll.value) {
          ...
          pendingInitialScrollTask.value = { type: 'bottom' };
          await loadMessages();
        }
      });
  } else {
...
const handleLoadOlder = async () => {
  if (!didInitialScroll.value || loadingOlder.value || listMode.value === 'fragment') {
    return;
  }
  const el = scrollContainer.value;
...
watch(messageList,
  async () => {
    const task = pendingInitialScrollTask.value;
    if (!task) {
      return;
    }
    ...
    if (task.type === 'bottom') {
      ...
      await scrollToMessage(scrollContainer.value, task.messageID, {
        block: 'center',
        behavior: 'instant',
      }).catch(() => {});
      highlightMessage({ messageID: task.messageID, duration: 3000 });
      setPendingLocateMessage(null);
    }
    await completeInitialScrollForEmptyList();
  },
  { flush: 'post' },
);
...
<template>
  <div
    class="message-list"
    :class="{ 'message-list--initializing': !didInitialScroll }"
  >
    <div
    ...
    >
      <ObserverView
        v-if="listMode === 'latest' && didInitialScroll"
